...
static int getai(const char *host, int port, struct addrinfo **ai)
{
    ...
    if (ssh_is_ipaddr(host) == 1) {
        ...
        SSH_LOG(SSH_LOG_FUNCTIONS, "host %s matches an IP address", host);
                                      ...
                                      const char *bind_addr, int port)
{
    ...
    for (itr = ai; itr != NULL; itr = itr->ai_next) {
        ...
        // improve windows connect timeout
#ifndef _WIN32
        ...
        if (rc == -1 && errCode != 0)// && errCode != WSAEWOULDBLOCK)
        {
            char* message = NULL;
            FormatMessage(FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS,
                NULL, errCode, MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT), (LPSTR)&message, 0, NULL);
            auto len = strlen(message);
            if (len > 1 && message[len - 2] == '\r' && message[len - 1] == '\n') message[len - 2] = '\0';
            ssh_set_error(session, errCode, "%s", message);
            LocalFree(message);
            ssh_connect_socket_close(s);
            s = -1;
            continue;
