...
SSH_PACKET_CALLBACK(ssh_packet_userauth_success)
{
  ...
  SSH_LOG(SSH_LOG_DEBUG, "Received SSH_USERAUTH_SUCCESS");
...
SSH_PACKET_CALLBACK(ssh_packet_userauth_pk_ok) {
    ...
    if (session->auth.state == SSH_AUTH_STATE_KBDINT_SENT) {
        ...
        SSH_LOG(SSH_LOG_DEBUG, "Assuming SSH_USERAUTH_PK_OK");
        rc = SSH_PACKET_USED;
    } else {
        session->auth.state = SSH_AUTH_STATE_ERROR;
        SSH_LOG(SSH_LOG_DEBUG, "SSH_USERAUTH_PK_OK received in wrong state");
        rc = SSH_PACKET_USED;
    }
