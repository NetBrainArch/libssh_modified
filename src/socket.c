                            ...
                            void *v_s)
{
    ...
    if (revents & POLLOUT) {
        ...
        if (s->state == SSH_SOCKET_CONNECTING) {
            SSH_LOG(SSH_LOG_FUNCTIONS, "Received POLLOUT in connecting state");
                                           ...
                                           uint32_t len)
{
    ...
    if (s->poll_handle) {
        SSH_LOG(SSH_LOG_FUNCTIONS, "Enabling POLLOUT for socket");
                       ...
                       const char *bind_addr)
{
    ...
    SSH_LOG(SSH_LOG_FUNCTIONS, "Nonblocking connection socket: %d", fd);
    if (fd == SSH_INVALID_SOCKET) {
        return SSH_ERROR;
    }
    ssh_socket_set_fd(s,fd);
